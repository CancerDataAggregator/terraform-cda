## sql sa
resource "google_service_account" "cda_sql_sa" {
...
resource "google_project_iam_member" "sql_sa_role" {
  ...
  member     = "serviceAccount:${google_service_account.cda_sql_sa[0].email}"
  depends_on = [var.dependencies]
}
...
resource "google_service_account_key" "sql_sa_key" {
  ...
  provider           = google.target
  service_account_id = google_service_account.cda_sql_sa[0].name
  depends_on         = [var.dependencies]
}
...
resource "vault_generic_secret" "sql_sa_key" {
  ...
  path       = "${local.vault_path}/cda-sql-sa"
...
## api sa
resource "google_service_account" "cda_api_sa" {
  count        = var.enable ? 1 : 0
...
resource "google_project_iam_member" "api_sa_role" {
  ...
  member     = "serviceAccount:${google_service_account.cda_api_sa[0].email}"
  depends_on = [var.dependencies]
}
...
resource "google_service_account_key" "api_sa_key" {
  ...
  provider           = google.target
  service_account_id = google_service_account.cda_api_sa[0].name
  depends_on         = [var.dependencies]
}
...
resource "vault_generic_secret" "api_sa_key" {
  ...
  depends_on = [var.dependencies]
  provider   = vault.target
  path       = "${local.vault_path}/cda-api-sa"
...
## test-runner-sa
resource "google_service_account" "cda_test_runner_sa" {
  count        = var.enable ? 1 : 0
  provider     = google.target
  project      = var.google_project
...
resource "google_project_iam_member" "test_runner_sa_role" {
  ...
  member     = "serviceAccount:${google_service_account.cda_test_runner_sa[0].email}"
  depends_on = [var.dependencies]
}
...
resource "google_service_account_key" "test_runner_sa_key" {
  ...
  provider           = google.target
  service_account_id = google_service_account.cda_test_runner_sa[0].name
  depends_on         = [var.dependencies]
}
